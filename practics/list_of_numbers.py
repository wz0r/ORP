...
class Shape(object):
    def __init__(self, *sides):
        ...
        if (sides_qty < 1) and (sides_qty > 4):
            raise ValueError('Ошибка при инициализации фигуры! Кол-во сторон {}, '
                             'а должно быть в диапазоне от 1 до 4.'.format(sides_qty))
        else:
            if sides_qty == 1:
    ...
    def __init__(self, *sides):
        ...
        if sides_qty != 3:
            raise ValueError('Ошибка при инициализации треугольника! Кол-во сторон {}, '
                             'а должно быть 3.'.format(sides_qty))
        else:
            self.sides = [n for n in sides]
        ...
        if two_sides_sum <= larger_side:
            raise ValueError('Ошибка при инициализации треугольника! '
                             'Со сторонами такой длины сторон построить треугольник невозможно!')
